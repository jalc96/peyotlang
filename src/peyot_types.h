...
internal Operator *new_operator(Memory_pool *allocator, PEYOT_TOKEN_TYPE type, str op1, str op2, str return_type) {
...
internal u32 hash(Operator *op) {
    ...
    u32 result = (((u32)op->type) * hash(op->operand_1) + ((u32)op->type) * hash(op->operand_2)) * hash(op->return_type);
    return result;
}
...
internal void print(Operator *op) {
    ...
    printf("%.*s: %.*s, %.*s -> %.*s", STR_PRINT(ops), STR_PRINT(op1), STR_PRINT(op2), STR_PRINT(return_type));
}
...
internal Operator *get(Operator_table *table, Operator *op) {
    Operator *result = 0;
    u32 index = get_operator_index(op);
